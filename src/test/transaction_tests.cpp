    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if (test[0].type() == array_type)
        {
            ...
            BOOST_FOREACH(Value& input, inputs)
            {
                ...
                mapprevOutScriptPubKeys[COutPoint(uint256(vinput[0].get_str()), vinput[1].get_int(),vinput[2].get_int64())] = ParseScript(vinput[2].get_str());
            }
            if (!fValid)
            {
                BOOST_ERROR("Bad test: " << strTest);
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    // Cccoin: IsDust() is disabled in favor of the per-dust output fee
