...
#if defined(QWEBHISTORYINTERFACE)
...
void HistoryManager::addHistoryItem(const HistoryItem &item)
{
    ...
    if (globalSettings->testAttribute(QWebSettings::PrivateBrowsingEnabled))
        return;
#endif
    //LogPrintf("HistoryManager::addHistoryItem\n");
...
{ 
    //LogPrintf("HistoryManager::updateHistoryItem url:%s, title %s \n",url.toString().toStdString().c_str(),title.toStdString().c_str());
    for (int i = 0; i < m_history.count(); ++i) {
       // LogPrintf("HistoryManager::updateHistoryItem mhistory%i ,url:%s \n",i,m_history.at(i).url.toStdString().c_str());
        if (url == m_history.at(i).url) {
           // LogPrintf("HistoryManager::updateHistoryItem history found \n");
...
{
    loadSettings();
    boost::filesystem::path path= GetDataDir() / "history";
    TryCreateDirectory(path);
    //LogPrintf("HistoryManager::load() path %s\n", path.string());
    QString dir =QString().fromStdString(path.string());
    QFile historyFile(dir + QLatin1String("/history"));
...
void HistoryManager::save()
{
    ...
    boost::filesystem::path path= GetDataDir() / "history";
    TryCreateDirectory(path);
    //LogPrintf("HistoryManager::save() path %s\n", path.string());
    QString directory =QString().fromStdString(path.string());    
...
{
    //LogPrintf("HistoryModel::entryAdded\n");
...
QVariant HistoryModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DecorationRole:
        if (index.column() == 0) {
            BitcoinGUI* gui=BitcoinApplication::instance()->getWindow();
            return gui->icon(item.url);
        }
    }
    return QVariant();
}
...
{
    //LogPrintf("HistoryMenu::activated \n");
...
{
    if (!m_history) {
        m_history = BitcoinGUI::historyManager();
...
HistoryDialog::HistoryDialog(QWidget *parent, HistoryManager *setHistory) : QDialog(parent)
{
    ...
    if (!history)
        history = BitcoinGUI::historyManager();
    setupUi(this);
