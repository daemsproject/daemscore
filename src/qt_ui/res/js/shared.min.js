...
var show_adv = false;
var adv_rule;
var symbol_btc = {code: "CCC", symbol: "CCC", name: "CCcoin", conversion: satoshi, symbolAppearsAfter: true, local: false};
var symbol_local = {conversion: 0, symbol: "$", name: "U.S. dollar", symbolAppearsAfter: false, local: true, code: "USD"};
        ...
        function f() {
            try {
                if (b % 2 == 0) {
                    ...
                    var g = "ws://ws.blockchain.info/inv"
                }
                ++b;
...
function BlockFromJSON(a) {
    return{hash: a.hash, time: a.time, blockIndex: a.blockIndex, height: a.height, txIndex: a.txIndexes, totalBTCSent: a.totalBTCSent, foundBy: a.foundBy, size: a.size}
}
function TransactionFromJSON(a) {
    return{hash: a.hash, size: a.size, txIndex: a.tx_index, time: a.time, inputs: a.inputs, out: a.out, blockIndex: a.block_index, result: a.result, blockHeight: a.block_height, balance: a.balance, double_spend: a.double_spend, note: a.note, setConfirmations: function (b) {
...
function setupSymbolToggle() {
    ...
    $("span[data-c]").unbind("mouseenter mouseleave").mouseenter(function () {
        (function (a) {
            ...
            if (!a.data("tooltip")) {
                $.ajax({timeout: 60000, type: "GET", dataType: "text", url: root + "frombtc", data: {value: a.data("c"), currency: symbol_local.code, time: a.data("time"), textual: true, nosavecurrency: true}, success: function (b) {
                        if (!b) {
                            return
                        }
                        a.tooltip({placement: "bottom", html: false, trigger: "manual", title: b});
                        if (a.is(":hover")) {
                            a.tooltip("show")
                        }
                    }, error: function (b) {
...
function showID (id) {
        //return "<div title='"+id+"'style='float:left'>"+id.substr(0, 10) + "......"+id.substr(-2)+"</div>";
...
function IsLevel2Domain(d){
    ...
    //console.log(d.substring(d.indexOf(".")+1));
...
function parseTx(tx,IDs) {
        ...
        if (tx.vin[0].coinbase) {
            ...
            for (var j in tx.vout) {
                var outid = tx.vout[j].scriptPubKey.addresses[0];
                var isOwnID = false;
                for (var k in IDs)
                    if (IDs[k] == outid)
                        isOwnID = true;
