...
static QString textForPermissionType(QWebPage::Feature type)
{
    switch (type) {
    ...
    //case QWebPage::MediaAudioCapture:
    //    return QObject::tr("your microphone");
    //case QWebPage::MediaVideoCapture:
    //    return QObject::tr("your camera");
    //case QWebPage::MediaAudioVideoCapture:
...
void FeaturePermissionBar::requestPermission(QWebFrame* securityOrigin, QWebPage::Feature feature)
{
    m_securityOrigin = securityOrigin;
    m_feature = feature;
    m_messageLabel->setText(tr("%1 wants to use %2.").arg(securityOrigin->requestedUrl().host()).arg(textForPermissionType(feature)));
    show();
    // Ease in
...
{
    emit featurePermissionProvided(m_securityOrigin, m_feature, QWebPage::PermissionDeniedByUser);
}

void FeaturePermissionBar::permissionGranted()
{
    emit featurePermissionProvided(m_securityOrigin, m_feature, QWebPage::PermissionGrantedByUser);
}
