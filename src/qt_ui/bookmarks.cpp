...
void BookmarksManager::load()
{
    if (m_loaded)
        return;
    m_loaded = true;
    boost::filesystem::path path= GetDataDir() / "bookmarks";
    TryCreateDirectory(path);
    LogPrintf("BookmarksManager::load() path %s\n", path.string());
    QString dir =QString().fromStdString(path.string());//  QStandardPaths::writableLocation(QStandardPaths::DataLocation);
...
void BookmarksManager::save() const
{
    ...
    boost::filesystem::path path= GetDataDir() / "bookmarks";
    TryCreateDirectory(path);
    LogPrintf("BookmarksManager::save() path %s\n", path.string());
    QString dir =QString().fromStdString(path.string());//  QStandardPaths::writableLocation(QStandardPaths::DataLocation);
...
QVariant BookmarksModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DecorationRole:
        if (index.column() == 0) {
            ...
            return gui->icon(bookmarkNode->url);
        }
    }
    ...
    , m_bookmarksManager(bookmarkManager)
{
    ...
    if (!m_bookmarksManager)
        m_bookmarksManager = BitcoinGUI::bookmarksManager();
    setupUi(this);
...
void BookmarksToolBar::build()
{
    ...
    for (int i = 0; i < m_bookmarksModel->rowCount(m_root); ++i) {
        ...
        if (m_bookmarksModel->hasChildren(idx)) {
            ...
            menu->setStyleSheet("QToolButton::menu-button {width: 100px;}");
