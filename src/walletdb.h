...
public:
    ...
    DBErrors LoadWallet(CWallet* pwallet,CKeyID id);    
    bool GetDefaultWallet(CKeyID& id);
    bool GetDefaultWallet(std::string& strAddress);
    bool SetDefaultWallet(const std::string& strAddress);
    bool SetDefaultWallet(const CKeyID& id);
    bool SetCurrentWallet(const CKeyID& id);
    bool SetCurrentWallet(const std::string& strAddress);
    bool GetCurrentWallet(CKeyID& id);
    bool GetCurrentWallet(std::string& strAddress);
    bool IsCurrentWallet(const CKeyID& id);
    bool GetWalletList(std::vector<std::string>& vIds)const;    
    bool IsWalletExists(const CKeyID& id)const;
    bool GetWalletName(const std::string& strNameIn,std::string& strNameOut)const;
    bool GetWalletName(const CKeyID& id,std::string& strName)const;
    bool SwitchToWallet(const CKeyID& id,CCryptoKeyStore* keyStore);
